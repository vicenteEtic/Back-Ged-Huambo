...
class PositionSeed extends Seeder
{
    ...
    /**
     * Cargos (funções exercidas) — conceito distintos das categorias do quadro.
     * Formato: [nome, código]
     */
    ...
    public function run(): void
    {
        ...
        foreach (self::QUADRO as $i => [$category, $name, $code, $salary]) {
        ...
        // Remove apenas categorias fora do quadro actual (cargos nunca são removidos aqui)
        $removed = Position::where('type', Position::TYPE_CATEGORIA)
            ->whereNotIn('code', array_column(self::QUADRO, 2))
            ->delete();
        if ($removed > 0) {
