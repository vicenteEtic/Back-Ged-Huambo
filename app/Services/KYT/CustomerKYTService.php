...
class CustomerKYTService
{
    ...
    private function checkHighCapitalIncrease(Entities $customer): void
    {
        ...
        $description = sprintf(
            "KYT_HIGH_CAPITAL_INCREASE: Aumento significativo de capital (%0.2f%%) em %d dias. " .
    ...
    private function checkEarlyRedemption(Entities $customer): void
    {
        ...
        foreach ($policies as $policy) {
            ...
            $description = sprintf(
                "KYT_EARLY_REDEMPTION: Cancelamento/resgate após %d dias da apólice %s (%s). " .
    ...
    private function checkHighPremiumLowRisk(Entities $customer): void
    {
        ...
        foreach ($policies as $policy) {
            ...
            $description = sprintf(
                "KYT_HIGH_PREMIUM_LOW_RISK: Prémio elevado (%0.2f%% do capital) em produto de baixo risco. " .
    ...
    private function checkMultipleShortPolicies(Entities $customer): void
    {
        ...
        $description = sprintf(
            "KYT_MULTIPLE_SHORT_POLICIES: %d apólices de curta duração (3–6 meses) nos últimos 12 meses. " .
    ...
    private function checkPolicyChurning(Entities $customer): void
    {
        ...
        $description = sprintf(
            "KYT_POLICY_CHURNING: %d ciclos de cancelamento e nova subscrição em curto espaço de tempo. " .
    ...
    private function checkRapidPolicyReplacement(Entities $customer): void
    {
        ...
        for ($i = 1; $i < $policies->count(); $i++) {
            ...
            $description = sprintf(
                "KYT_RAPID_POLICY_REPLACEMENT: Apólice %s substituída por %s em %d dias.",
